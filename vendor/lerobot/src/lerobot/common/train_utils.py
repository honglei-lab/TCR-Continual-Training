...
def save_training_state(
    ...
    *,
    ...
    # Every data-parallel rank has its own Python/NumPy/CPU/CUDA RNG stream.
    # A single rank-0 file cannot reproduce stochastic flow/noise draws after
    # DDP resume, so persist one collision-free file per global rank.
    process_rank = dist.get_rank() if dist.is_initialized() else 0
    save_rng_state(save_dir, f"rng_state_rank{process_rank}.safetensors")

    if is_main_process():  # ONE grouped gate for the whole rank-0-only region
        save_training_metadata(step, save_dir, cfg)
        # Keep the historical filename for readers of older checkpoint layouts.
...
def load_process_rng_state(training_state_dir: Path) -> None:
    """Load this process's RNG state, with legacy rank-0 fallback."""
    process_rank = dist.get_rank() if dist.is_initialized() else 0
    filename = f"rng_state_rank{process_rank}.safetensors"
    if not (training_state_dir / filename).is_file():
        filename = RNG_STATE
    load_rng_state(training_state_dir, filename)
...
def resume_before_prepare(cfg: TrainPipelineConfig) -> int:
    ...
    load_process_rng_state(training_state_dir)
