...
def _episode_task_keys(full_dataset: LeRobotDataset, base_episodes: list[int]) -> dict[int, str]:
    """Return one stable task key per episode for stratified train/eval splitting.

    Newer episode metadata may include a ``tasks`` list directly.  LIBERO v3
    metadata predates that column but stores a scalar ``task_index`` on every
    frame.  In the latter case, derive the episode mapping from the raw frame
    table and fail if any episode contains more than one task index.
    """
    episode_columns = set(full_dataset.meta.episodes.column_names)
    if "tasks" in episode_columns:
        episode_tasks = full_dataset.meta.episodes["tasks"]
        return {
            ep_idx: str(episode_tasks[ep_idx][0] if episode_tasks[ep_idx] else "")
            for ep_idx in base_episodes
        }

    frame_columns = set(full_dataset.hf_dataset.column_names)
    if not {"episode_index", "task_index"}.issubset(frame_columns):
        raise ValueError(
            "Cannot stratify eval_split: neither episode metadata 'tasks' nor "
            "frame-level ('episode_index', 'task_index') columns are available."
        )
    frame_tasks = full_dataset.hf_dataset.select_columns(
        ["episode_index", "task_index"]
    ).to_pandas()
    grouped = frame_tasks.groupby("episode_index", sort=False)["task_index"].agg(
        ["min", "max"]
    )
    inconsistent = grouped[grouped["min"] != grouped["max"]]
    if not inconsistent.empty:
        bad = [int(value) for value in inconsistent.index[:20]]
        raise ValueError(f"Episodes contain multiple task_index values: {bad}")
    mapping = {
        int(ep_idx): f"task_index:{int(row['min'])}" for ep_idx, row in grouped.iterrows()
    }
    missing = sorted(set(base_episodes) - set(mapping))
    if missing:
        raise ValueError(f"Missing frame-level task_index for episodes: {missing[:20]}")
    return {ep_idx: mapping[ep_idx] for ep_idx in base_episodes}
...
def make_train_eval_datasets(
    ...
    episode_tasks = _episode_task_keys(full_dataset, base_episodes)
    task_to_episodes: dict[str, list[int]] = {}
    for ep_idx in base_episodes:
        task_key = episode_tasks[ep_idx]
