...
def _cycle_from_iterator(iterable, first_iterator):
    """Cycle an iterable while reusing an already-created first iterator.

    Constructing a PyTorch DataLoader iterator consumes RNG for its worker base
    seed even with ``num_workers=0``.  Resume therefore creates the iterator
    first, restores the checkpoint RNG afterward, and enters this cycle without
    constructing a second iterator before the first resumed batch.
    """
...
def _emit_exact_resume_trace(batch: dict[str, Any], step: int) -> None:
    """Emit compact per-rank batch/RNG hashes for preflight diagnosis only."""
    if os.environ.get("PI05_EXACT_RESUME_TRACE") != "1":
        return
    ...
    for key in ("index", "episode_index", "task_index"):
        ...
        if isinstance(value, torch.Tensor):
            ...
            payload[f"batch_{key}_sha256"] = hashlib.sha256(raw).hexdigest()
            payload[f"batch_{key}_first"] = int(value.reshape(-1)[0].item())
            payload[f"batch_{key}_last"] = int(value.reshape(-1)[-1].item())
    payload["cpu_rng_sha256"] = hashlib.sha256(
        torch.get_rng_state().numpy().tobytes()
    ).hexdigest()
    if torch.cuda.is_available():
        payload["cuda_rng_sha256"] = hashlib.sha256(
            torch.cuda.get_rng_state().cpu().numpy().tobytes()
        ).hexdigest()
    print("PI05_EXACT_RESUME_TRACE " + json.dumps(payload, sort_keys=True), flush=True)
...
def train(cfg: TrainPipelineConfig):
    ...
    first_dl_iter = iter(dataloader)
    if cfg.resume:
        load_process_rng_state(cfg.checkpoint_path / TRAINING_STATE_DIR)
        if is_main_process():
            logging.info("Restored checkpoint RNG after DataLoader iterator construction")
    dl_iter = _cycle_from_iterator(dataloader, first_dl_iter)
    restore_rng_after_first_resumed_batch = cfg.resume
    if restore_rng_after_first_resumed_batch and (
        cfg.num_workers != 0 or cfg.dataset.image_transforms.enable
    ):
        raise RuntimeError(
            "Exact stochastic resume after lazy DataLoader construction requires "
            "num_workers=0 and disabled dataset image transforms."
        )

    for _ in range(step, cfg.steps):
        step_start = time.perf_counter()
        batch = next(dl_iter)
        if restore_rng_after_first_resumed_batch:
            # Accelerate's prepared DataLoader creates its underlying PyTorch
            # iterator lazily on first next(), which consumes a CPU RNG draw for
            # the worker base seed even when num_workers=0.  Formal training has
            # image transforms disabled, so fetch the deterministic batch first
            # and then restore the serialized stochastic-training boundary.
            load_process_rng_state(cfg.checkpoint_path / TRAINING_STATE_DIR)
            restore_rng_after_first_resumed_batch = False
            if is_main_process():
                logging.info("Restored checkpoint RNG after first resumed batch fetch")
        _emit_exact_resume_trace(batch, step)
        ...
        if cfg.save_checkpoint and is_saving_step:
            ...
            accelerator.wait_for_everyone()
            # Saving model/optimizer/sidecars may execute Torch operations after
            # the RNG files have been captured.  Reload the just-written
            # rank-specific state so an uninterrupted run and a resumed run
            # leave the checkpoint boundary from the exact same RNG state.
            load_process_rng_state(checkpoint_dir / TRAINING_STATE_DIR)
            if is_main_process():
                logging.info("Restored rank-specific RNG after checkpoint save")
