...
def save_rng_state(save_dir: Path, filename: str = RNG_STATE) -> None:
    rng_state_dict = serialize_rng_state()
    flat_rng_state_dict = flatten_dict(rng_state_dict)
    save_file(flat_rng_state_dict, save_dir / filename)


def load_rng_state(save_dir: Path, filename: str = RNG_STATE) -> None:
    flat_rng_state_dict = load_file(save_dir / filename)
